...
public class ReadPuzzleInput {
    ...
    private List<Integer> getLinesAsIntegers(String inputPath) {
        ...
        try {
            List<String> input = Files.readAllLines(Paths.get("src/main/resources/day" + adventDay + "/" + inputPath));
            return input.stream()
    ...
    private List<String> getLines(String inputPath) {
        try {
            return Files.readAllLines(Paths.get("src/main/resources/day" + adventDay + "/" + inputPath));
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
